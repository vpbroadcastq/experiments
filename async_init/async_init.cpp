...
public:
	...
	expensive& get() {
		if (m_async_complete) {
			// Once the async is done it will never run again; there is no reason to bother
			// with taking the lock
			if (!m_expensive) {
				this->m_who_init = thread_name::main;
				m_expensive = std::make_unique<expensive>();
			}
		} else {
			// If the async hasn't completed we have to lock, since the async might stomp
			// us in the middle of our work to do the init
			m_mtx.lock();
			if (!m_expensive) {
				this->m_who_init = thread_name::main;
				m_expensive = std::make_unique<expensive>();
...
private:
	...
	std::atomic<bool> m_async_complete {false};
	std::mutex m_mtx;
};
